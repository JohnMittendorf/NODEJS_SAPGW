...
sap.ui.core.Core = function() {
	...
	function getOS(userAgent){
		...
		var platform = /[(]([a-zA-TV-Z]+);\s(?:[U]?[;]?)([\D]+)((?:\d[._])+\d)/;
		...
		if (result){
			...
			if (result[0].match(appleDevices)){
				...
				// currently we only support iOS and Android, everything else will be ignored, if more platforms should be supported, logic can be placed here
				return;
			}
		}
	}
